...
def annotate_metadata(cell_names_dict, metadata_df, ann_col_name):
    """
    Annotate metadata DataFrame with cell names based on coordinates.
    
    Parameters:
    - cell_names_dict: Dictionary with cell names as keys and lists of indices as values.
    - metadata_df: DataFrame containing metadata with an index that matches the cell names.
    
    Returns:
    - Annotated DataFrame with a new column 'cell_name'.
    """
    # Initialize a new column for cell names
    metadata_df[ann_col_name] = None
    
    # Iterate through the dictionary and assign cell names
    for section_name, indices in cell_names_dict.items():
        metadata_df.loc[indices, ann_col_name] = section_name
